...
void hip_sine_init(void)
{
	...
	/* START - Lingyuan - 03/2015 */
	
	/* lsi string for hip */
	struct in_addr lsi;
	hip_get_default_hit(&hit);
	inet_ntop(AF_INET, lsi, lsiaddr, sizeof(lsiaddr));
	...
	sprintf(cmd, "sudo ip route add table hip0 1.0.0.0/8 dev hip0 proto kernel scope link src %s", lsiaddr);
...
void *hip_pref_listener(void *arg)
{
	...
	hip_sine_init(); /* Lingyuan - 03/2015 */
				...
				if (strcmp(buf, "pref") == 0)
				{
					...
					else
					{
						...
						{/*
							...
							/* Lingyuan - 03/2015*/
							gateway = get_router_str(hip_preferred_ifname);
