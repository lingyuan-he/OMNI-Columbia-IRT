 ...
 */
static int xfrm_policy_modify(struct rtnl_handle *rth, int cmd,
                              const struct in6_addr *id_our,
                              const struct in6_addr *id_peer,
 ...
 */
static int xfrm_sa_flush(struct rtnl_handle *rth)
{
    struct {
        struct nlmsghdr          n;
 ...
 * @return zero on success and non-zero on failure
 */
static int xfrm_policy_flush(struct rtnl_handle *rth)
{
    struct {
        struct nlmsghdr n;
    } req   = { { 0 } };
 ...
 */
static int xfrm_policy_delete(struct rtnl_handle *rth,
 ...
 * @return 0 if successful, negative on error
 */
static int xfrm_state_modify(struct rtnl_handle *rth,
                             ...
                             const int enckey_len, const int aalg,
 ...
 */
static int xfrm_state_delete(struct rtnl_handle *rth,
 ...
 */
static int calc_sp_prefix(const struct in6_addr *src_id,
                          int use_full_prefix)
{
    uint8_t prefix;
...
/**
 * A wrapper to xfrm_policy_flush()
 *
 * @return zero on success and negative on error
 */
int hip_flush_all_policy(void)
{
    return xfrm_policy_flush(hip_xfrmapi_nl_ipsec);
}

/**
 * A wrapper to xfrm_sa_flush()
 *
 * @return zero on success and negative on error
 */
int hip_flush_all_sa(void)
{
    return xfrm_sa_flush(hip_xfrmapi_nl_ipsec);
}
                   ...
                   const int direction, struct hip_hadb_state *entry)
{
    ...
    if (direction == HIP_SPI_DIRECTION_OUT) {
        xfrm_state_delete(hip_xfrmapi_nl_ipsec, peer_addr, spi, AF_INET6,
                    ...
                    struct hip_hadb_state *entry)
{
    ...
    HIP_IFE(xfrm_state_modify(hip_xfrmapi_nl_ipsec, XFRM_MSG_NEWSA,
                              saddr, daddr, src_hit, dst_hit, spi,
...
{
    int     err    = 0;
    uint8_t prefix = calc_sp_prefix(src_id, use_full_prefix);

    /* XX FIXME: remove the proto argument */
    HIP_IFE(xfrm_policy_modify(hip_xfrmapi_nl_ipsec, XFRM_MSG_NEWPOLICY,
                               dst_id, src_id,
                               src_addr, dst_addr,
                            ...
                            const int use_full_prefix)
{
    ...
    xfrm_policy_delete(hip_xfrmapi_nl_ipsec, dst_hit, src_hit,
                       XFRM_POLICY_IN, prefix, AF_INET6);
    xfrm_policy_delete(hip_xfrmapi_nl_ipsec, src_hit, dst_hit,
